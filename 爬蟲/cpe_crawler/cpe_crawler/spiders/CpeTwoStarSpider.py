...
class CpePreviousSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        soup = BeautifulSoup(two_star, 'html.parser')

        two_star_url = soup.select('table > tbody > tr > td > a')
        two_star_url = [x.get('href') for x in two_star_url]

        two_star_id = soup.select('table > tbody > tr > td')
        two_star_id = [x.text.split('(')[1].strip(r')|\xa0') for x in two_star_id if '(' in x.text]
        two_star_id = [x.split('\xa0')[0] for x in two_star_id]
        two_star_id = [x.split(')')[0] for x in two_star_id]
        two_star_id = [x.strip('UVA') for x in two_star_id]

        for index in range(len(two_star_id)):
            item = CpeCrawlerItem()

            item['id'] = two_star_id[index]

            yield scrapy.Request(url=response.urljoin(two_star_url[index]), callback=self.parseCode, meta={'question': item})

    def parseCode(self, response):
        FOLDER_BASE = 'D:\\CPE\\二星題\\'
